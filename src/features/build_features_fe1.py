...
class AddCols(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X["avg_data_compl_usg_local_3m"] = X[
            ["data_compl_usg_local_m2", "data_compl_usg_local_m3", "data_compl_usg_local_m4"]
        ].mean(axis=1)

        usage = X[["data_compl_usg_local_m2", "data_compl_usg_local_m3", "data_compl_usg_local_m4"]]
...
def main():
    ...
    df.drop(
        columns=["telephone_number"], inplace=True
    )  # tried to use it as index but it has duplicates
